...
class SingleContainerRuntimeTest(unittest.TestCase):
    ...
    def test_search_api_and_research_degrade_when_disabled(self):
        ...
        tested_get = client.get("/api/search/test", params={"query": "current news"})
        searched = client.post("/api/search", json={"query": "current news"})
        research = ResearchSearcher().search_query("current news")

        self.assertEqual(tested.status_code, 200)
        self.assertEqual(tested_get.status_code, 200)
        self.assertEqual(tested_get.json()["message"], "Web search is disabled in this runtime.")
        self.assertEqual(
            tested.json(),
            {
                "success": False,
