...
class LLMRegistryTests(unittest.TestCase):
    def test_registry_supports_multiple_local_and_api_configs(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            configs = [
                LLMConfig(id="local", name="Local", provider="ollama", model="qwen:7b",
                          base_url="http://localhost:11434"),
                LLMConfig(id="hosted", name="Hosted", provider="openai_compatible", model="model-a",
    ...
    def test_disabled_active_config_falls_back_to_enabled_config(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            path.write_text(json.dumps({"active_id": "off", "llms": [
                {"id": "off", "name": "Off", "provider": "ollama", "model": "a",
                 "base_url": "http://localhost:1", "enabled": False},
                {"id": "on", "name": "On", "provider": "ollama", "model": "b",
                 "base_url": "http://localhost:2", "enabled": True},
            ]}), encoding="utf-8")
