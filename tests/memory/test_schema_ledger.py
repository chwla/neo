...
from tests.memory.factories import DATABASE_IDENTITY, OWNER_A
...
def test_empty_database_upgrade_creates_complete_versioned_schema(tmp_path) -> None:
    ...
    try:
        ...
        assert set(MEMORY_TABLES) <= tables
        assert MEMORY_LEDGER_TABLE in tables
        assert memory_migration_state(engine).current_revision == MEMORY_CURRENT_REVISION
        assert set(MEMORY_TABLES).isdisjoint(Base.metadata.tables)
    finally:
        engine.dispose()


def test_upgrade_is_idempotent(memory_engine) -> None:
    first = memory_migration_state(memory_engine)
    upgrade_memory(
        memory_engine,
        owner_id=OWNER_A,
        database_identity=DATABASE_IDENTITY,
    )
    second = memory_migration_state(memory_engine)
    assert first == second
    with memory_engine.connect() as connection:
        count = connection.scalar(text(f"SELECT count(*) FROM {MEMORY_LEDGER_TABLE}"))
...
def test_upgrade_preserves_minimal_legacy_schema_and_data(tmp_path) -> None:
    ...
    try:
        ...
        upgrade_memory(engine, owner_id=OWNER_A, database_identity=DATABASE_IDENTITY)
...
def test_explicit_downgrade_removes_only_and_preserves_legacy(tmp_path) -> None:
    ...
    try:
        ...
        tables = set(inspect(engine).get_table_names())
        assert not (set(MEMORY_TABLES) & tables)
        assert MEMORY_LEDGER_TABLE not in tables
        with engine.connect() as connection:
...
def test_wrong_owner_or_database_identity_fails_closed(memory_engine) -> None:
    try:
        upgrade_memory(
            memory_engine,
        ...
        assert str(exc) == "memory_owner_database_binding_mismatch"
    else:
        raise AssertionError("wrong owner binding was accepted")


def test_unmanaged_partial_schema_fails_safely(tmp_path) -> None:
    engine = build_engine(f"sqlite:///{tmp_path / 'partial.db'}")
    try:
        with engine.begin() as connection:
            connection.execute(text("CREATE TABLE memory_records (id TEXT PRIMARY KEY)"))
        try:
            upgrade_memory(engine, owner_id=OWNER_A, database_identity=DATABASE_IDENTITY)
        except MemoryMigrationError as exc:
            assert str(exc).startswith("unmanaged_memory_tables:")
        else:
            raise AssertionError("partial unmanaged memory schema was accepted")
    finally:
        engine.dispose()
...
def test_unknown_migration_revision_fails_safely(tmp_path) -> None:
    ...
    try:
        with engine.begin() as connection:
            connection.execute(
                text(
                    f"CREATE TABLE {MEMORY_LEDGER_TABLE} ("
        ...
        try:
            ...
            assert str(exc).startswith("unsupported_memory_revisions:")
        else:
...
def test_changed_revision_checksum_fails_safely(memory_engine) -> None:
    with memory_engine.begin() as connection:
        connection.execute(
            text(
                f"UPDATE {MEMORY_LEDGER_TABLE} SET revision_checksum = 'corrupt' "
                "WHERE revision = :revision"
            ),
            {"revision": MEMORY_CURRENT_REVISION},
        )
    try:
        upgrade_memory(
            memory_engine,
        ...
        assert str(exc) == "memory_revision_checksum_mismatch"
    else:
