...
from tests.memory.helpers import DATABASE_IDENTITY, OWNER_A, actor, candidate, source
...
def test_replacement_failure_at_every_sql_stage_rolls_back_everything(
    ...
    with Session(phase2_engine) as session:
        ...
        assert session.scalar(select(func.count(MemoryCandidate.id))) == 1
        assert session.scalar(select(func.count(MemoryRelation.id))) == 0
        assert session.scalar(select(func.count(MemorySource.id))) == 1
        assert session.scalar(select(func.count(MemoryOutbox.id))) == 1
...
def test_tombstone_stage_failure_rolls_back_forget(
    ...
    assert result.outcome is MemoryOutcome.FAILED
    with Session(phase2_engine) as session:
        record = session.get(MemoryRecord, str(original.active_memory_ids[0]))
