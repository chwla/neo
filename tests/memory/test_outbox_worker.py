...
def test_canonical_commit_only_queues_outbox_and_worker_processes_post_commit(tmp_path) -> None:
    ...
    try:
        row = services.recall.session.get(MemoryRecord, str(memory_id))
...
def test_pending_survives_restart_and_active_lease_is_exclusive(tmp_path) -> None:
    ...
    try:
        ...
        restarted = MemoryOutboxProcessor(
            services.recall.session.get_bind(),
            owner_id=services.recall.repository.owner_id,
            database_identity=services.recall.repository.database_identity,
            fts_index=services.fts,
...
def test_fts_success_vector_failure_is_independent_and_retryable(tmp_path) -> None:
    ...
    services.processor = MemoryOutboxProcessor(
        services.recall.session.get_bind(),
        owner_id=services.recall.repository.owner_id,
        database_identity=services.recall.repository.database_identity,
        fts_index=fts,
    ...
    try:
        ...
        deliveries = list(
            services.recall.session.scalars(
                select(MemoryOutboxDelivery).order_by(MemoryOutboxDelivery.target)
            )
        )
        ...
        derived = {
            ...
            for item in services.recall.session.scalars(select(MemoryDerivedState))
        }
...
def test_permanent_failure_dead_letters_and_explicit_requeue_recovers(tmp_path) -> None:
    ...
    try:
        ...
        retry = services.processor.lease_batch(worker_id="worker-b")
        services.processor.process_batch(retry)
        states = list(services.recall.session.scalars(select(MemoryDerivedState.state)))
        services.recall.session.rollback()
...
def test_old_canonical_remove_event_cannot_delete_newer_derived_revision(tmp_path) -> None:
    ...
    try:
        ...
        services.recall.session.expire_all()
        assert services.recall.session.get(MemoryRecord, record.id).revision == 2
    finally:
        services.close()
...
def test_vector_delete_runs_without_embedding_provider(tmp_path) -> None:
    ...
    try:
        ...
        delete_only = MemoryOutboxProcessor(
            services.recall.session.get_bind(),
            owner_id=owner_id,
            database_identity=services.recall.repository.database_identity,
            vector_index=services.vector,
