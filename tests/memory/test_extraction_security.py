...
from app.services.memory.model_schema import parse_model_output
...
from tests.memory.mutation_helpers import OWNER_B
...
def test_strict_model_schema_rejects_owner_and_canonical_target_ids() -> None:
    ...
    assertion["memory_chat_id"] = "00000000-0000-4000-8000-000000000099"
    try:
...
def test_hallucinated_value_and_assistant_span_are_rejected(tmp_path) -> None:
    ...
    harness, extraction, _diagnostics = extraction_harness(tmp_path / "hallucination", model=model)
    result = run_text(
        extraction,
        harness,
        text,
    ...
    harness, extraction, _diagnostics = extraction_harness(tmp_path / "assistant", model=model)
    result = run_text(
        extraction,
        harness,
        user_text,
...
def test_mixed_paragraph_reaches_fallback_and_keeps_durable_facts_over_temporary_ones(
    tmp_path,
) -> None:
    text = (
        "I am a data engineer. I drank tea today, have a headache right now, might watch "
        "a movie tonight, and am temporarily tired. I use Rust for work."
    )
    temporary_values = (
        "drank tea today",
    ...
    temporary = [
        _assertion(
            ...
            value,
            message_id="mixed",
            proposal_id=f"temporary-{index}",
            durability="temporary",
        )
        for index, value in enumerate(temporary_values)
    ]
    role = _assertion(
        text,
    ...
    model = FixtureExtractionModel({text: _response(*temporary, role, rust)})
    harness, extraction, _diagnostics = extraction_harness(tmp_path, model=model)
    ...
    assert result.status is ExtractionStatus.APPLIED
    assert result.model_summary.called
    assert result.model_summary.capped_count == 2
    assert model.call_count == 1
    state = sql_state(harness.database_path)
    assert {(row["memory_type"], row["display_text"]) for row in state["records"]} == {
        ("identity", "I am a data engineer"),
        ("knowledge", "I use Rust for work"),
    }
    assert not any(value in str(state) for value in temporary_values)
    ...
    try:
        ...
        assert {item.memory.display_text for item in recalled.items} == {
            "I am a data engineer",
            "I use Rust for work",
        }
    finally:
...
def test_automatic_candidate_cap_is_four_and_explicit_batch_can_exceed_four(tmp_path) -> None:
    ...
    harness, extraction, _diagnostics = extraction_harness(tmp_path / "cap", model=model)
    capped = run_text(
        extraction,
        harness,
        text,
    ...
    harness, extraction, _diagnostics = extraction_harness(tmp_path / "batch")
    batch = run_text(
        extraction,
        harness,
        batch_text,
...
def test_ambiguous_correction_persists_review_candidate_without_mutation(tmp_path) -> None:
    harness, base_extraction, _diagnostics = extraction_harness(tmp_path / "base")
    run_text(
        base_extraction,
        harness,
