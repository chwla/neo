...
from tests.memory.mutation_helpers import OWNER_A, OWNER_B, memory_harness, video_goal
...
def _counts(path) -> tuple[int, int, int]:
    ...
    try:
        return (
            ...
            connection.execute("SELECT count(*) FROM memory_tombstones").fetchone()[0],
            connection.execute("SELECT count(*) FROM memory_operations").fetchone()[0],
        )
    finally:
        connection.close()
...
def _source_state(path, memory_id: UUID) -> dict[str, object]:
    ...
    try:
        record = dict(
            connection.execute(
                "SELECT id, owner_id, status, revision FROM memory_records WHERE id = ?",
                (str(memory_id),),
            ).fetchone()
        )
        sources = [
            ...
            for row in connection.execute(
                ...
                "detachment_reason FROM memory_sources WHERE memory_id = ? ORDER BY id",
                (str(memory_id),),
            ).fetchall()
        ]
        operations = [
            row[0]
            for row in connection.execute(
                "SELECT operation_kind FROM memory_operations ORDER BY created_at, id"
            ).fetchall()
        ]
        outbox = [
            row[0]
            for row in connection.execute(
                "SELECT event_kind FROM memory_outbox ORDER BY created_at, id"
            ).fetchall()
        ]
        return {
            "record": record,
...
def _create_with_two_supports(harness):
    adapter = GenericMemoryAdapter(harness.coordinator)
    ...
    created = adapter.create(
        ...
        idempotency_key=MemoryIdempotency.manual(OWNER_A, "source-create-a"),
    )
    assert created.mutation and str(created.mutation.active_memory_ids[-1])
    reconfirmed = adapter.create(
        second_context,
        video_goal("create short Instagram reels clearly"),
        idempotency_key=MemoryIdempotency.manual(OWNER_A, "source-create-b"),
    )
    assert reconfirmed.mutation and reconfirmed.mutation.outcome.value == "reconfirmed"
...
def test_archive_restore_forget_and_permanent_erasure_use_commands(tmp_path) -> None:
    ...
    created = adapter.create(
        harness.context,
        video_goal("create short Instagram reels clearly"),
        idempotency_key=MemoryIdempotency.manual(OWNER_A, "lifecycle-create"),
    )
    assert created.mutation and str(created.mutation.active_memory_ids[-1])
    ...
    archived = adapter.archive(
        harness.context,
        target,
        idempotency_key=MemoryIdempotency.manual(OWNER_A, "archive"),
    )
    assert archived.mutation and archived.mutation.outcome.value == "archived"
    restored = adapter.restore(
        harness.context,
        TargetRevision(memory_id=target.memory_id, expected_revision=2),
        idempotency_key=MemoryIdempotency.manual(OWNER_A, "restore"),
    )
    assert restored.mutation and restored.mutation.outcome.value == "restored"
    forgotten = adapter.forget(
        harness.context,
        TargetRevision(memory_id=target.memory_id, expected_revision=3),
        idempotency_key=MemoryIdempotency.manual(OWNER_A, "forget"),
    )
    assert forgotten.mutation and forgotten.mutation.outcome.value == "forgotten"
    assert _counts(harness.database_path) == (1, 1, 4)

    second = adapter.create(
        harness.context,
        video_goal("create tutorial videos"),
        idempotency_key=MemoryIdempotency.manual(OWNER_A, "erase-create"),
    )
    assert second.mutation and str(second.mutation.active_memory_ids[-1])
    erased = adapter.erase_permanently(
        harness.context,
        TargetRevision(
            memory_id=UUID(str(second.mutation.active_memory_ids[-1])),
            expected_revision=1,
        ),
        idempotency_key=MemoryIdempotency.manual(OWNER_A, "erase"),
    )
    assert erased.mutation and erased.mutation.outcome.value == "erased_permanently"
...
def test_source_detach_preserves_memory_and_other_persisted_support_idempotently(tmp_path) -> None:
    ...
    source_changes = MemorySourceChangeCoordinator(adapter)
...
def test_source_not_found_and_owner_mismatch_are_stable_typed_results(tmp_path) -> None:
    ...
    mismatch = harness.coordinator.detach_source(
        ...
        DetachMemorySourceCommand(
            owner_id=OWNER_B,
            idempotency_key=MemoryIdempotency.source_change(
