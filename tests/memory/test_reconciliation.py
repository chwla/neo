...
from app.services.memory.queries import RecallQuery
...
from tests.memory.recall_helpers import add_memory, query_context
...
def _maintenance(services) -> MemoryIndexMaintenance:
    return MemoryIndexMaintenance(
        services.recall.session.get_bind(),
        owner_id=services.recall.repository.owner_id,
        database_identity=services.recall.repository.database_identity,
...
def test_missing_vector_reconciliation_repairs_once_and_second_run_is_clean(
    ...
    try:
        ...
        assert not missing_coverage.ready
...
def test_reconciliation_classifies_inactive_expired_ghost_and_model_drift(
    ...
    try:
        _drain(services)
        model_record = services.recall.session.get(MemoryRecord, str(model_id))
        ...
        services.recall.session.execute(
            update(MemoryRecord)
            .where(MemoryRecord.id == str(inactive_id))
            .values(status="archived")
        )
        services.recall.session.execute(
            update(MemoryRecord)
            .where(MemoryRecord.id == str(expired_id))
            .values(expires_at=datetime.now(UTC) - timedelta(seconds=1))
        )
        services.recall.session.execute(
            update(MemoryFtsDocument)
            .where(MemoryFtsDocument.memory_id == str(model_id))
            .values(derived_schema_version="obsolete-derived-schema")
        )
        services.recall.session.execute(
            update(MemoryVectorPoint)
            .where(MemoryVectorPoint.memory_id == str(model_id))
            .values(
        ...
        assert second.repairs_queued == 0
        metadata = services.vector.get_metadata(services.recall.repository.owner_id, str(model_id))
...
def test_reconciliation_repairs_stale_hashes_and_detects_pending_current(
    ...
    try:
        ...
        services.recall.session.execute(
            update(MemoryFtsDocument)
            .where(MemoryFtsDocument.memory_id == str(memory_id))
            .values(content_hash="0" * 64)
        )
        services.recall.session.execute(
            update(MemoryVectorPoint)
            .where(MemoryVectorPoint.memory_id == str(memory_id))
            .values(content_hash="0" * 64)
        )
        services.recall.session.commit()
        maintenance = _maintenance(services)
        ...
        assert not stale_coverage.ready
        _drain(services, "stale-repair-worker")
        assert maintenance.reconcile().repairs_queued == 0

        delivery = services.recall.session.scalar(
            select(MemoryOutboxDelivery).where(MemoryOutboxDelivery.target == "vector")
        )
        event = services.recall.session.get(MemoryOutbox, delivery.event_id)
...
def test_done_target_specific_event_does_not_claim_unrequested_target_is_missing(
    ...
    services = index_services(harness)
    try:
        original = services.recall.session.scalar(
            select(MemoryOutbox).where(MemoryOutbox.memory_id == str(memory_id))
        )
        services.recall.session.delete(original)
        services.recall.session.commit()
        ...
        assert services.vector.get_metadata(services.recall.repository.owner_id, str(memory_id))
        assert (
            services.fts.get_metadata(services.recall.repository.owner_id, str(memory_id)) is None
        )
...
def test_owner_rebuild_preserves_canonical_state_and_recreates_indexes(tmp_path) -> None:
    ...
    try:
        ...
        recall = CanonicalRecallService(
            services.recall.repository,
            flags=services.recall.harness.coordinator.flags,
            fts_index=services.fts,
        ...
        before = {
            ...
            for row in services.recall.session.scalars(select(MemoryRecord))
        }
        maintenance = _maintenance(services)
        ...
        _drain(services)
        services.recall.session.expire_all()
        after = {
            row.id: (row.revision, row.status)
            for row in services.recall.session.scalars(select(MemoryRecord))
        }
...
def test_vector_provider_outage_during_rebuild_preserves_canonical_and_fts(tmp_path) -> None:
    ...
    try:
        ...
        assert services.fts.get_metadata(services.recall.repository.owner_id, str(memory_id))
        assert (
            services.vector.get_metadata(services.recall.repository.owner_id, str(memory_id))
            is None
        )
...
def test_reconciliation_pages_derived_only_ghosts_and_repairs_persisted_state(
    ...
    try:
        _drain(services)
        owner_id = services.recall.repository.owner_id
        source_record = services.recall.session.get(MemoryRecord, str(memory_ids[0]))
        ...
        services.recall.session.expire_all()
        assert services.recall.session.get(MemoryRecord, str(ghost_id)) is None
...
def test_maintenance_uses_configured_batch_retry_and_alert_policy(tmp_path) -> None:
    ...
    try:
        settings = Settings(
            memory_reconciliation_batch_size=1,
            memory_retry_max_attempts=9,
            memory_alert_oldest_pending_seconds=17,
            memory_alert_dead_letter_count=2,
            memory_alert_min_coverage_ratio=0.8,
            memory_alert_consecutive_provider_failures=4,
            memory_alert_stale_ghost_rate=0.2,
            memory_alert_lease_expiration_rate=0.3,
        )
...
def test_global_rebuild_requires_privilege_and_health_contains_no_plaintext(tmp_path) -> None:
    ...
    try:
        ...
        with pytest.raises(PermissionError, match="privileged_memory"):
        ...
        structured_unhealthy = MemoryIndexMaintenance(
            services.recall.session.get_bind(),
            owner_id=services.recall.repository.owner_id,
            database_identity=services.recall.repository.database_identity,
            fts_index=services.fts,
