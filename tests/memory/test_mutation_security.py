...
from app.services.memory.tombstones import TombstoneSnapshot, resurrection_blocked
...
def test_sensitive_record_candidate_command_and_evidence_are_opaque(
    ...
    with Session(phase2_engine) as session:
        record = session.get(MemoryRecord, str(result.active_memory_ids[0]))
        operation = session.get(MemoryOperation, str(result.operation_id))
        stored_candidate = session.get(MemoryCandidate, str(result.candidate_id))
        stored_source = session.scalar(
            select(MemorySource).where(MemorySource.operation_id == str(result.operation_id))
        )
        ...
        outbox_payloads = session.scalars(select(MemoryOutbox.event_payload_json)).all()
...
def test_prohibited_material_is_rejected_before_candidate_or_record_persistence(
    ...
    with Session(phase2_engine) as session:
        assert session.scalar(select(func.count(MemoryRecord.id))) == 0
        assert session.scalar(select(func.count(MemoryCandidate.id))) == 0
        operation = session.scalar(select(MemoryOperation))
...
def test_repository_content_guard_ignores_structural_uuids_but_not_payloads() -> None:
    ...
    MemoryRepository._reject_prohibited_material(structural)
    with pytest.raises(MemoryProhibitedContentError):
        MemoryRepository._reject_prohibited_material({"canonical_value": "4111 1111 1111 1111"})
...
def test_cross_owner_target_fails_as_not_found(
    ...
    upgrade_memory(other_engine, owner_id=OWNER_B, database_identity=other_identity)
    try:
