...
from app.models.memory import MemoryRecord, MemoryUsageEvent
from app.services.embeddings import ValidatedMemoryEmbeddingProvider
from app.services.memory.contracts import ReplacementAuthority, Sensitivity, TargetRevision
...
from app.services.memory.index_contracts import VectorCandidate
from app.services.memory.indexes import DerivedDocumentBuilder
from app.services.memory.prompt import RecallPromptOrchestrator, repository_usage_recorder
...
from app.services.memory.versions import VECTOR_METADATA_VERSION
...
from tests.memory.mutation_helpers import OWNER_B, video_goal
...
def test_stale_superseded_predecessor_is_dropped_for_active_successor(tmp_path) -> None:
    ...
    try:
        old = services.recall.session.get(MemoryRecord, str(old_id))
        ...
        assert services.recall.session.get(MemoryRecord, str(old_id)).usage_count == 0
    finally:
        services.close()


def test_ghost_and_wrong_owner_hits_do_not_consume_or_suppress_local_result(tmp_path) -> None:
    harness, adapter = index_harness(tmp_path / "owner-a")
    local_id = add_memory(
        adapter,
        harness,
        key="semantic-local",
    ...
    services = index_services(harness)
    other_harness, other_adapter = index_harness(
        tmp_path / "owner-b",
        owner_id=OWNER_B,
        profile_id="owner-b",
    )
    ...
    other_services = index_services(other_harness)
    try:
        local = services.recall.session.get(MemoryRecord, str(local_id))
        ...
        other_record = other_services.recall.session.get(MemoryRecord, str(other_id))
        ...
        assert all(str(item.owner_id) == services.recall.repository.owner_id for item in repairs)
        ...
        assert (
            other_services.vector.get_metadata(
                other_services.recall.repository.owner_id, str(other_id)
            )
            is not None
        )
        other_services.recall.session.expire_all()
        assert other_services.recall.session.get(MemoryRecord, str(other_id)).usage_count == 0
...
def test_hash_mismatch_is_rejected_and_current_upsert_repair_is_scheduled(tmp_path) -> None:
    ...
    try:
        ...
        result = recall.recall(
            RecallQuery(
                context=query_context(services.recall, domains=frozenset({"learning"})),
                text="quantum orchid",
            )
        )
        ...
        assert services.recall.session.get(MemoryRecord, str(memory_id)).revision == 1
    finally:
        services.close()
...
def test_embedding_and_vector_outages_degrade_to_lexical_and_deterministic(tmp_path) -> None:
    ...
    try:
        ...
        prompt = RecallPromptOrchestrator(
            recall,
            usage_recorder=repository_usage_recorder(services.recall.repository),
        ).build(
            RecallQuery(
                context=query_context(
                    services.recall,
        ...
        direct_service = DirectMemoryAnswerService(
        ...
        assert direct_service.last_canonical_ids == (memory_id,)
        assert (
            services.recall.session.scalar(
                select(MemoryUsageEvent).where(
                    MemoryUsageEvent.memory_id == str(memory_id),
                    MemoryUsageEvent.purpose == "direct_answer",
                )
            )
            is not None
        )
        ...
        deterministic = recall.recall(
            RecallQuery(
                context=query_context(services.recall, mode=RecallMode.DETERMINISTIC),
...
def test_expired_semantic_hit_is_rejected_without_usage_and_queues_delete(tmp_path) -> None:
    ...
    try:
        ...
        assert repairs[0].action == "delete"
        services.recall.session.expire_all()
        assert services.recall.session.get(MemoryRecord, str(memory_id)).usage_count == 0
    finally:
        services.close()
...
def test_sensitive_semantic_hit_is_rejected_and_persisted_vector_is_deleted(tmp_path) -> None:
    ...
    services = index_services(harness)
    try:
        donor = services.recall.session.get(MemoryRecord, str(donor_id))
        ...
        services.recall.session.expire_all()
        sensitive = services.recall.session.get(MemoryRecord, str(sensitive_id))
...
def test_hybrid_merge_respects_record_text_budgets_and_usage_matches_serialized_ids(
    ...
    try:
        ...
        services.recall.session.expire_all()
        for memory_id in memory_ids:
            usage = services.recall.session.get(MemoryRecord, str(memory_id)).usage_count
...
def test_semantic_domain_gate_beats_high_score_pin_and_duplicate_ids_merge(tmp_path) -> None:
    ...
    try:
        services.recall.session.execute(
            update(MemoryRecord)
            .where(MemoryRecord.id == str(unrelated_id))
            .values(pinned=True, importance=10)
        )
        services.recall.session.commit()
        local = services.recall.session.get(MemoryRecord, str(local_id))
        unrelated = services.recall.session.get(MemoryRecord, str(unrelated_id))
