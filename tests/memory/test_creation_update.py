...
from app.models.memory import MemoryCandidate, MemoryRecord, MemorySource
...
def test_exact_typed_duplicate_reconfirms_without_new_record(
    ...
    with Session(phase2_engine) as session:
        ...
        record = session.get(MemoryRecord, str(first.active_memory_ids[0]))
        assert record is not None and record.revision == 2
        assert session.scalar(select(func.count(MemorySource.id))) == 2
...
def test_similar_but_different_value_in_exclusive_slot_needs_review(
    ...
    with Session(phase2_engine) as session:
        assert session.scalar(select(func.count(MemoryRecord.id))) == 1
        active = session.get(MemoryRecord, str(first.active_memory_ids[0]))
        assert active is not None and active.canonical_payload == "create short tutorial videos"
        review = session.scalar(
            select(MemoryCandidate).where(MemoryCandidate.state == "needs_review")
        )
