...
from tests.memory_v2.factories import OWNER_A, OWNER_B, operation, record, uuid_string
...
def test_two_active_exclusive_records_same_owner_slot_are_rejected(memory_v2_engine) -> None:
    factory = _factory(memory_v2_engine)
    first = factory()
    try:
...
def test_same_exclusive_slot_is_allowed_for_different_owners(memory_v2_session) -> None:
...
def test_independent_additive_goals_are_allowed(memory_v2_session) -> None:
...
def test_duplicate_active_fingerprint_same_owner_is_rejected(memory_v2_session) -> None:
    memory_v2_session.add(operation(number=100))
    memory_v2_session.add_all(
        [
...
def test_same_active_fingerprint_is_allowed_for_different_owners(memory_v2_session) -> None:
    memory_v2_session.add_all(
        [
            operation(owner_id=OWNER_A, number=100),
            operation(owner_id=OWNER_B, number=101),
...
def test_cross_owner_source_insert_is_rejected(memory_v2_session) -> None:
...
def test_cross_owner_relation_and_self_relation_are_rejected(memory_v2_engine) -> None:
    factory = _factory(memory_v2_engine)
    session = factory()
    try:
        session.add_all(
...
def test_invalid_normal_sensitive_or_prohibited_record_is_rejected(
    memory_v2_session, mutator
) -> None:
    memory_v2_session.add(operation(number=100))
    item = record(number=200)
    mutator(item)
    memory_v2_session.add(item)
    with pytest.raises(IntegrityError):
        memory_v2_session.flush()


def test_opaque_sensitive_payload_shape_is_accepted(memory_v2_session) -> None:
    memory_v2_session.add(operation(number=100))
...
def test_invalid_record_states_ranges_and_versions_are_rejected(
    memory_v2_session, field: str, value
) -> None:
    memory_v2_session.add(operation(number=100))
    item = record(number=200)
...
def test_prohibited_candidate_cannot_be_persisted(memory_v2_session) -> None:
    memory_v2_session.add(
        MemoryCandidateV2(
            id=uuid_string(500),
            owner_id=OWNER_A,
...
def test_operation_idempotency_is_unique_per_owner(memory_v2_session) -> None:
    memory_v2_session.add_all(
        [
...
def test_invalid_outbox_state_is_rejected(memory_v2_session) -> None:
    memory_v2_session.add(
        MemoryOutboxV2(
            id=uuid_string(600),
            owner_id=OWNER_A,
...
def test_tombstone_has_no_plaintext_columns_and_expiry_must_follow_creation(
    memory_v2_engine, memory_v2_session
) -> None:
    columns = {
        column["name"] for column in inspect(memory_v2_engine).get_columns("memory_tombstones_v2")
    }
    assert not ({"canonical_payload", "display_text", "memory_text", "canonical_value"} & columns)

    now = datetime.now(UTC)
    memory_v2_session.add(operation(number=100, operation_kind=MemoryOperationKind.FORGET.value))
    memory_v2_session.add(
        MemoryTombstoneV2(
            id=uuid_string(700),
            owner_id=OWNER_A,
