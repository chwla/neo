...
from app.models.memory_v2 import MemoryCandidateV2, MemoryRecordV2
...
def test_owner_status_expiry_predicates_are_in_canonical_sql(tmp_path) -> None:
    ...
    try:
        services.session.execute(
            ...
            .where(MemoryRecordV2.id == str(expired))
            .values(expires_at=datetime.now(UTC) - timedelta(seconds=1))
        )
        services.session.execute(
            update(MemoryRecordV2)
            .where(MemoryRecordV2.id == str(archived))
        ...
        assert "memory_records_v2.owner_id =" in sql
        assert "memory_records_v2.status = 'active'" in sql
        assert "memory_records_v2.expires_at IS NULL" in sql
        assert "memory_records_v2.expires_at >" in sql
...
def test_scoped_relevance_domain_and_pin_cannot_bypass_gates(tmp_path) -> None:
    ...
    try:
        services.session.execute(
            update(MemoryRecordV2)
            .where(MemoryRecordV2.id == str(pinned))
...
def test_every_nonserving_lifecycle_shape_is_excluded_from_canonical_recall(tmp_path) -> None:
    ...
    try:
        ...
        assert "memory_candidates_v2" not in serving_sql
    finally:
        services.close()
