...
from app.services.memory_v2.prompt import STABLE_MEMORY_POLICY
...
def test_secure_wrapper_suppression_and_final_id_usage_match(tmp_path) -> None:
    ...
    try:
        ...
        old_row = services.session.get(MemoryRecordV2, str(old))
        malicious_row = services.session.get(MemoryRecordV2, str(malicious))
...
def test_incognito_disabled_and_usage_failure_fail_closed(tmp_path) -> None:
    ...
    try:
        ...
        row = services.session.get(MemoryRecordV2, str(memory_id))
