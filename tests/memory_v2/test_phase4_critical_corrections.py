...
from app.services.memory_v2.extraction_contracts import CandidateAction, ExtractionStatus
...
def test_preference_correction_replaces_old_global_style(tmp_path) -> None:
    from app.services.memory_v2.adapters import GenericMemoryV2Adapter, StructuredMemoryInput
...
def test_category_correction_uses_authorized_prior_user_span(tmp_path) -> None:
    ...
    from app.services.memory_v2.idempotency import MemoryV2Idempotency
    from tests.memory_v2.phase4_helpers import extraction_input
...
def test_pure_location_retraction_archives_instead_of_creating_not_pune(
    ...
    GenericMemoryV2Adapter(harness.coordinator).create(
        ...
        idempotency_key=MemoryV2Idempotency.manual(harness.context.execution.owner_id, "seed-pune"),
    )
    result = run_text(
        extraction,
        harness,
...
def test_resolver_targets_all_conflicts_in_same_exclusive_slot() -> None:
    ...
    from app.services.memory_v2.extraction_contracts import NormalizedExtractionCandidate
