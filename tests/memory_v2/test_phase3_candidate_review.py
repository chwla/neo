...
from app.services.memory_v2.contracts import MemoryUpdatePatch, TargetRevision
...
def _active_rows(path) -> list[tuple[str, str, str, str, int]]:
    ...
    try:
        return connection.execute(
            ...
            "FROM memory_records_v2 WHERE status = 'active' ORDER BY memory_type, slot_key"
        ).fetchall()
    finally:
        connection.close()
...
def test_review_accept_duplicate_and_compatible_refinement_match_direct_commands(tmp_path) -> None:
    ...
    review = CandidateReviewV2Adapter(harness.coordinator)
    ...
    assert accepted.coordination and accepted.coordination.compatibility
    target = TargetRevision(
        memory_id=UUID(accepted.coordination.compatibility.active_memory_id),
        expected_revision=2,
    )
...
def test_review_category_correction_is_explicit_typed_replacement(tmp_path) -> None:
    ...
    created = generic.create(
        ...
        idempotency_key=MemoryV2Idempotency.manual(OWNER_A, "mistaken-category"),
    )
    ...
    corrected = CandidateReviewV2Adapter(harness.coordinator).apply(
        harness.context,
...
def test_review_explicit_merge_uses_supplied_positive_value(tmp_path) -> None:
    ...
    first = generic.create(
        ...
        idempotency_key=MemoryV2Idempotency.manual(OWNER_A, "merge-first"),
    )
    second = generic.create(
        harness.context,
        learning_goal("practice Python projects"),
        idempotency_key=MemoryV2Idempotency.manual(OWNER_A, "merge-second"),
    )
    assert first.compatibility and first.compatibility.active_memory_id
    ...
    merged = CandidateReviewV2Adapter(harness.coordinator).apply(
        harness.context,
        ...
        targets=(
            TargetRevision(
                memory_id=UUID(first.compatibility.active_memory_id),
                expected_revision=1,
            ),
            TargetRevision(
                memory_id=UUID(second.compatibility.active_memory_id), expected_revision=1
            ),
        ),
    )
