...
def test_archive_and_restore_into_empty_slot(mutation_service, phase2_engine) -> None:
    ...
    with Session(phase2_engine) as session:
        ...
        event_kinds = list(session.scalars(select(MemoryOutboxV2.event_kind)))
...
def test_unsafe_restore_fails_and_restore_as_replacement_creates_new_version(
    ...
    with Session(phase2_engine) as session:
        old = session.get(MemoryRecordV2, str(historical_id))
        previous_current = session.get(MemoryRecordV2, str(current.active_memory_ids[0]))
...
def test_forget_blocks_automatic_resurrection_and_allows_explicit_reconfirmation(
    ...
    with Session(phase2_engine) as session:
        old = session.get(MemoryRecordV2, str(memory_id))
        ...
        tombstone = session.scalar(select(MemoryTombstoneV2))
        assert tombstone is not None
        assert tombstone.explicitly_reconfirmed
        assert value not in tombstone.fingerprint_digest
        assert (
            session.scalar(
                select(func.count(MemoryRecordV2.id)).where(MemoryRecordV2.status == "active")
            )
            == 1
        )
        assert "tombstone_expiry" in list(session.scalars(select(MemoryOutboxV2.event_kind)))
...
def test_expired_tombstone_does_not_block_recreation(mutation_service, phase2_engine) -> None:
    ...
    with phase2_engine.begin() as connection:
        connection.execute(
            update(MemoryTombstoneV2).values(
...
def test_permanent_erasure_removes_value_provenance_tombstone_and_blocking_fingerprint(
    ...
    with Session(phase2_engine) as session:
        ...
        assert (
            session.scalar(
                select(func.count(MemoryCandidateV2.id)).where(
                    MemoryCandidateV2.applied_operation_id == str(created.operation_id)
                )
            )
            == 0
        )
        creating_operation = session.get(MemoryOperationV2, str(created.operation_id))
        ...
        reconciliation = session.scalar(
            select(MemoryOutboxV2).where(MemoryOutboxV2.event_kind == "reconciliation_request")
        )
