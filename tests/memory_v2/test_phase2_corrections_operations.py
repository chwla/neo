...
from app.models.memory_v2 import MemoryRecordV2, MemoryRelationV2
...
def test_grounded_same_slot_structured_correction_is_deterministic(
    ...
    with Session(phase2_engine) as session:
        active = session.get(MemoryRecordV2, str(replacement.active_memory_ids[0]))
        old = session.get(MemoryRecordV2, str(original.active_memory_ids[0]))
...
def test_explicit_domain_change_rebuilds_semantic_slot(mutation_service, phase2_engine) -> None:
    ...
    with Session(phase2_engine) as session:
        record = session.get(MemoryRecordV2, str(changed.active_memory_ids[0]))
...
def test_category_correction_and_preference_scopes_remain_distinct(
    ...
    with Session(phase2_engine) as session:
        scoped = session.get(MemoryRecordV2, str(corrected.active_memory_ids[0]))
        global_record = session.get(MemoryRecordV2, str(global_style.active_memory_ids[0]))
...
def test_one_replacement_can_clean_up_multiple_explicit_predecessors(
    ...
    with Session(phase2_engine) as session:
        ...
        assert (
            session.scalar(
                select(func.count(MemoryRecordV2.id)).where(MemoryRecordV2.status == "superseded")
            )
            == 2
        )
        assert (
            session.scalar(
                select(func.count(MemoryRelationV2.id)).where(
...
def test_explicit_compatible_merge_uses_supplied_value_without_text_concatenation(
    ...
    with Session(phase2_engine) as session:
        record = session.get(MemoryRecordV2, str(merged.active_memory_ids[0]))
        assert record is not None and record.canonical_payload == merged_value
        assert "\n" not in record.canonical_payload
        assert (
            session.scalar(
                select(func.count(MemoryRelationV2.id)).where(
                    MemoryRelationV2.relation_type == "merged_from"
                )
            )
            == 2
        )
...
def test_explicit_supersede_uses_existing_active_successor(mutation_service, phase2_engine) -> None:
    ...
    with Session(phase2_engine) as session:
        old = session.get(MemoryRecordV2, str(predecessor.active_memory_ids[0]))
