...
from tests.memory_v2.phase3_helpers import OWNER_A, phase3_harness, video_goal
...
def _state(path) -> dict[str, object]:
    ...
    try:
        records = connection.execute(
            ...
            "display_text, status, revision FROM memory_records_v2 "
        ...
        return {
            ...
            "relations": [
                ...
                for row in connection.execute(
                    "SELECT relation_type FROM memory_relations_v2 ORDER BY relation_type"
                ).fetchall()
            ],
            "operations": [
                tuple(row)
                for row in connection.execute(
                    "SELECT operation_kind, outcome FROM memory_operations_v2 ORDER BY created_at"
                ).fetchall()
            ],
            "sources": connection.execute("SELECT count(*) FROM memory_sources_v2").fetchone()[0],
            "tombstones": connection.execute(
                "SELECT count(*) FROM memory_tombstones_v2"
            ).fetchone()[0],
            "outbox": [
                tuple(row)
                for row in connection.execute(
                    "SELECT event_kind, state FROM memory_outbox_v2 ORDER BY event_kind, id"
                ).fetchall()
            ],
        }
    finally:
    ...
    for surface in ("generic", "typed", "chat", "import"):
        ...
        if surface == "generic":
            result = GenericMemoryV2Adapter(harness.coordinator).create(
                harness.context,
                item,
                idempotency_key=MemoryV2Idempotency.http(OWNER_A, "request-1", "create"),
            )
        elif surface == "typed":
            result = TypedMemoryV2Adapter(harness.coordinator).create_typed(
                harness.context,
                item,
                client_mutation_id="typed-1",
            )
        elif surface == "chat":
            result = ChatMemoryV2Adapter(harness.coordinator).apply_structured_candidate(
                harness.context,
                item,
            ...
            result = ImportMemoryV2Adapter(harness.coordinator).accept(
                harness.context,
                {
        ...
        outcomes.append(result.compatibility.outcome)
...
def test_sync_stream_and_cross_transport_retries_are_one_logical_operation(tmp_path) -> None:
    ...
    adapter = ChatMemoryV2Adapter(harness.coordinator)
    item = video_goal(NEW_GOAL)
    results = [
...
def test_equivalent_replacement_through_four_surfaces_has_same_canonical_result(
    ...
    generic = GenericMemoryV2Adapter(harness.coordinator)
    created = generic.create(
        harness.context,
        video_goal(OLD_GOAL),
        idempotency_key=MemoryV2Idempotency.manual(OWNER_A, f"old-{surface}"),
    )
    ...
    target = TargetRevision(
        ...
        expected_revision=created.compatibility.revision or 1,
    )
    item = video_goal(NEW_GOAL)
    if surface == "generic":
        result = generic.replace(
            ...
            idempotency_key=MemoryV2Idempotency.http(OWNER_A, "replacement", "replace"),
        )
    elif surface == "typed":
        result = TypedMemoryV2Adapter(harness.coordinator).replace(
            harness.context,
            item,
            (target,),
            authority=ReplacementAuthority.EXPLICIT_CORRECTION,
            idempotency_key=MemoryV2Idempotency.manual(OWNER_A, "typed-replacement"),
        )
    elif surface == "review":
        reviewed = CandidateReviewV2Adapter(harness.coordinator).apply(
            harness.context,
        ...
        result = ChatMemoryV2Adapter(harness.coordinator).apply_structured_replacement(
            harness.context,
            item,
            (target,),
            extraction_version="legacy-structured-v1",
    ...
    assert result.compatibility is not None
    assert result.compatibility.outcome == "replaced"
    state = _state(harness.database_path)
...
def test_candidate_review_rejection_and_ambiguity_do_not_mutate_canonical_state(tmp_path) -> None:
    ...
    adapter = CandidateReviewV2Adapter(harness.coordinator)
