...
from app.models.memory_v2 import MemoryOutboxV2, MemorySourceV2
from app.services.memory_v2.contracts import SourceKind
...
from tests.memory_v2.factories import DATABASE_IDENTITY, OWNER_A, operation, record, uuid_string
...
def test_healthy_fixture_checksums_and_invariant_report_are_deterministic(
    ...
    _seed_healthy(memory_v2_engine)

    identity = identify_database_owner(memory_v2_engine)
    assert identity.owner_id == OWNER_A
    assert identity.database_identity == DATABASE_IDENTITY
    assert run_sqlite_integrity_check(memory_v2_engine) == ("ok",)
    assert schema_checksum(memory_v2_engine) == schema_checksum(memory_v2_engine)
    assert canonical_data_checksum(memory_v2_engine, owner_id=OWNER_A) == canonical_data_checksum(
        memory_v2_engine, owner_id=OWNER_A
    )

    report = inspect_memory_v2_invariants(memory_v2_engine, owner_id=OWNER_A)
...
def test_backup_restores_to_equivalent_readable_database(memory_v2_engine, tmp_path) -> None:
    _seed_healthy(memory_v2_engine)
    expected_schema = schema_checksum(memory_v2_engine)
    expected_data = canonical_data_checksum(memory_v2_engine, owner_id=OWNER_A)
    destination = tmp_path / "backups" / "memory-v2-backup.db"

    manifest = create_sqlite_backup(memory_v2_engine, destination)
    ...
    try:
        ...
        assert inspect_memory_v2_invariants(restored, owner_id=OWNER_A).healthy
    finally:
        restored.dispose()


def test_invariant_report_detects_deliberately_corrupted_rows(memory_v2_engine) -> None:
    _seed_healthy(memory_v2_engine)
    ...
    try:
        ...
        connection.execute("DROP INDEX uq_memory_records_v2_active_exclusive_slot")
        connection.execute("DROP INDEX uq_memory_records_v2_active_fingerprint")
        connection.execute(
            ...
            "record_schema_version, legacy_id FROM memory_records_v2 WHERE id = ?",
            (uuid_string(201), uuid_string(200)),
        )
        connection.execute(
            "UPDATE memory_records_v2 SET display_text = NULL, created_by_operation_id = ? "
            "WHERE id = ?",
            (uuid_string(999), uuid_string(200)),
        )
        connection.execute(
            "UPDATE memory_sources_v2 SET memory_id = ? WHERE id = ?",
