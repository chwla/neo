...
from app.services.memory_v2.correction_resolver import CorrectionResolutionKind
...
from app.services.memory_v2.extraction_coordinator import (
    CurrentTurnOverrideBuilder,
    MemoryV2ExtractionCoordinator,
)
...
def test_prohibited_input_leaves_no_plaintext_or_durable_trace(
    ...
    harness, extraction, diagnostics = phase4_harness(tmp_path / kind, model=model)
...
def test_approved_sensitive_input_is_encrypted_and_result_safe(tmp_path, capsys, caplog) -> None:
    ...
    with sqlite3.connect(harness.database_path) as connection:
        record = connection.execute(
            ...
            "encrypted_display_payload FROM memory_records_v2"
        ).fetchone()
        candidate = connection.execute(
            "SELECT canonical_payload, display_text, encrypted_canonical_payload, "
            "encrypted_display_payload, target_hints_json, raw_output_hash "
            "FROM memory_candidates_v2"
        ).fetchone()
        operation = connection.execute(
            "SELECT normalized_command_json, encrypted_command_payload FROM memory_operations_v2"
        ).fetchone()
        sources = connection.execute(
            "SELECT redacted_excerpt, encrypted_excerpt FROM memory_sources_v2"
        ).fetchall()
        outbox = connection.execute("SELECT event_payload_json FROM memory_outbox_v2").fetchall()
    ...
    decrypted = ChatMemoryV2Adapter(harness.coordinator).list_active_memories(harness.context)
...
def test_sensitive_correction_suppresses_only_encrypted_predecessor(tmp_path) -> None:
    ...
    active = ChatMemoryV2Adapter(harness.coordinator).list_active_memories(harness.context)
...
def test_category_and_exact_duplicate_reconfirmations_suppress_nothing(tmp_path) -> None:
    ...
    category_extraction = MemoryV2ExtractionCoordinator(
...
def test_replacement_retraction_and_ambiguity_have_distinct_suppression(tmp_path) -> None:
    ...
    harness, extraction, _diagnostics = phase4_harness(tmp_path / "replace")
    ...
    harness, extraction, _diagnostics = phase4_harness(tmp_path / "retract")
    seeded = GenericMemoryV2Adapter(harness.coordinator).create(
        harness.context,
        StructuredMemoryInput(
        ...
        idempotency_key=MemoryV2Idempotency.manual(
    ...
    harness, base, _diagnostics = phase4_harness(tmp_path / "ambiguous")
    original = run_text(
        base,
        harness,
        f"I want to {old}.",
    ...
    ambiguous_extraction = MemoryV2ExtractionCoordinator(
        ChatMemoryV2Adapter(harness.coordinator), model=model
    )
    ambiguous = run_text(
...
def test_failure_disabled_and_ignored_paths_publish_empty_overrides(tmp_path) -> None:
    ...
    harness, extraction, _diagnostics = phase4_harness(
    ...
    harness, extraction, _diagnostics = phase4_harness(tmp_path / "timeout", model=timeout_model)
    timeout = run_text(
        extraction,
        harness,
        timeout_text,
    ...
    for index, text in enumerate(
        ...
        harness, extraction, _diagnostics = phase4_harness(tmp_path / f"ignored-{index}")
        _assert_empty_override(run_text(extraction, harness, text, message_id=f"ignored-{index}"))

    harness, extraction, _diagnostics = phase4_harness(tmp_path / "disabled")
    incognito = run_text(
        extraction,
        harness,
        "I use Go for work.",
...
def test_manual_fixture_output_and_retained_artifacts_are_sentinel_free(tmp_path) -> None:
    root = Path(__file__).resolve().parents[2]
    environment = {**os.environ, "TMPDIR": str(tmp_path)}
    completed = subprocess.run(
        [sys.executable, "scripts/manual_memory_v2_phase4.py", "--keep"],
        cwd=root,
        env=environment,
        capture_output=True,
        text=True,
    ...
    sensitive = "-".join(("P4", "SENSITIVE", "C71A9E4D0B62"))
    prohibited = "-".join(("P4", "PROHIBITED", "D84F2A710C95"))
    assert sensitive not in output
    assert prohibited not in output
    assert "prohibited_plaintext_leak_count=0" in output
    assert "sensitive_plaintext_log_leak_count=0" in output
    assert "sensitive_plaintext_artifact_leak_count=0" in output
    assert "sensitive_payload_encrypted=true" in output
    assert "category_reconfirm_suppressed_ids=[]" in output
    assert "ambiguous_conflict_suppression_authorized=false" in output
    retained = next(
        Path(line.split("=", 1)[1])
        for line in output.splitlines()
        if line.startswith("artifacts_retained=")
    )
    try:
        assert retained.is_dir()
        assert _artifact_count(retained, sensitive) == 0
        assert _artifact_count(retained, prohibited) == 0
    finally:
