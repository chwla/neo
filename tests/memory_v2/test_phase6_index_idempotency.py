...
from tests.memory_v2.phase5_helpers import add_memory
...
def test_repeated_vector_upsert_has_one_point_and_old_delete_cannot_remove_new_hash(
    ...
    try:
        ...
        services.phase5.session.refresh(record)
        ...
        metadata = services.vector.get_metadata(services.phase5.repository.owner_id, str(memory_id))
...
def test_delayed_worker_delete_cannot_remove_or_mark_newer_vector_deleted(tmp_path) -> None:
    ...
    try:
        ...
        state = services.phase5.session.scalar(
            select(MemoryDerivedStateV2).where(
                MemoryDerivedStateV2.owner_id == owner_id,
                MemoryDerivedStateV2.memory_id == str(memory_id),
                MemoryDerivedStateV2.target == DerivedTarget.VECTOR.value,
            )
        )
        ...
        event = services.phase5.session.scalar(
            select(MemoryOutboxV2).where(
                MemoryOutboxV2.id == str(result.event_id),
                MemoryOutboxV2.state == "done",
            )
        )
...
def test_fts5_namespace_returns_only_bounded_owner_hash_candidates(tmp_path) -> None:
    ...
    try:
        ...
        assert hits[0]["owner_id"] == services.phase5.repository.owner_id
...
def test_embedding_boundary_rejects_wrong_dimension_and_non_finite_values(vector, code) -> None:
    provider = ValidatedMemoryV2EmbeddingProvider(InvalidEmbeddingSource(vector), dimension=3)
...
def test_embedding_boundary_rejects_missing_provider_identity() -> None:
    with pytest.raises(ValueError, match="embedding_provider_identity_invalid"):
        ValidatedMemoryV2EmbeddingProvider(MissingEmbeddingIdentitySource(), dimension=3)
