...
class Settings(BaseSettings):
    ...
    memory_v2_canonical_query_enabled: bool = Field(default=False)
    memory_v2_lexical_recall_enabled: bool = Field(default=False)
    memory_v2_secure_prompt_enabled: bool = Field(default=False)
    memory_v2_direct_answer_reads_enabled: bool = Field(default=False)
    memory_v2_research_recall_enabled: bool = Field(default=False)
    memory_v2_legacy_read_compatibility: bool = Field(default=True)
    memory_v2_recall_max_records: int = Field(default=5, ge=1, le=20)
    memory_v2_recall_max_chars: int = Field(default=2400, ge=200, le=12000)
    memory_v2_recall_min_score: float = Field(default=0.18, ge=0, le=1)
    memory_v2_outbox_worker_enabled: bool = Field(default=False)
    memory_v2_fts_index_enabled: bool = Field(default=False)
    memory_v2_vector_index_enabled: bool = Field(default=False)
    memory_v2_semantic_recall_enabled: bool = Field(default=False)
    memory_v2_reconciliation_enabled: bool = Field(default=False)
    memory_v2_derived_health_routes_enabled: bool = Field(default=False)
    memory_v2_worker_lease_seconds: int = Field(default=60, ge=5, le=3600)
    memory_v2_worker_batch_size: int = Field(default=25, ge=1, le=500)
    memory_v2_worker_poll_seconds: float = Field(default=2, ge=0.1, le=300)
    memory_v2_retry_max_attempts: int = Field(default=5, ge=1, le=100)
    memory_v2_dead_letter_threshold: int | None = Field(default=None, ge=1, le=100)
    memory_v2_retry_base_seconds: int = Field(default=5, ge=1, le=3600)
    memory_v2_retry_max_seconds: int = Field(default=300, ge=1, le=86400)
    memory_v2_retry_jitter_seconds: int = Field(default=0, ge=0, le=3600)
    memory_v2_fts_candidate_limit: int = Field(default=50, ge=1, le=500)
    memory_v2_vector_candidate_limit: int = Field(default=50, ge=1, le=500)
    memory_v2_semantic_weight: float = Field(default=0.35, ge=0, le=1)
    memory_v2_semantic_cap: float = Field(default=1, ge=0, le=1)
    memory_v2_query_embedding_timeout_seconds: int = Field(default=5, ge=1, le=60)
    memory_v2_index_embedding_timeout_seconds: int = Field(default=30, ge=1, le=300)
    memory_v2_provider_cooldown_seconds: int = Field(default=60, ge=1, le=3600)
    memory_v2_reconciliation_batch_size: int = Field(default=250, ge=1, le=5000)
    memory_v2_alert_oldest_pending_seconds: int = Field(default=900, ge=1)
    memory_v2_alert_dead_letter_count: int = Field(default=1, ge=0)
    memory_v2_alert_min_coverage_ratio: float = Field(default=0.95, ge=0, le=1)
    memory_v2_alert_consecutive_provider_failures: int = Field(default=3, ge=1)
    memory_v2_alert_stale_ghost_rate: float = Field(default=0.05, ge=0, le=1)
    memory_v2_alert_lease_expiration_rate: float = Field(default=0.1, ge=0, le=1)
    memory_v2_embedding_model: str = Field(default="nomic-embed-text:latest")
    memory_v2_embedding_provider: str = Field(default="ollama")
    memory_v2_embedding_version: str = Field(default="1")
    memory_v2_embedding_dimension: int = Field(default=768, ge=1, le=65536)
