...
class ReliableWebSearchService:
    ...
    def run(self, request) -> dict:
        ...
        run = store.create_run(
            request.query, request.mode, search_plan, {"provider": response.provider}
        )
        if response.error or response.provider == "disabled":
            final = store.update_run(
                run["id"],
                status="degraded",
                error=safe(response.error or "Web search is disabled."),
                summary_text="Search is unavailable; no unsupported facts were produced.",
        ...
        for result in response.results[: request.max_sources]:
            ...
            breakdown = self._score(
                request.query, text, domain, official, request.freshness_required
            )
                ...
                safe(
                    {
                        ...
                        "credibility_score": breakdown["credibility"],
                        "freshness_score": breakdown["freshness"],
                        "relevance_score": breakdown["relevance"],
                        "final_score": breakdown["final"],
                        "metadata": {"provider": response.provider, "score_breakdown": breakdown},
                        "redaction_summary": {},
                    }
                ),
            )
            source = (
                store.update_source(
                    ...
                    relevance_score=breakdown["relevance"],
                    final_score=breakdown["final"],
                    metadata_json=store.json_text(
                        {"provider": response.provider, "score_breakdown": breakdown}
                    ),
                )
                or source
            )
            ...
            store.upsert_cache(
                {
                    ...
                    "fetched_text": source.get("fetched_text") or source["snippet"],
                    "metadata": source.get("metadata", {}),
                    "redaction_summary": source.get("redaction_summary", {}),
                }
            )
                ...
                evidence.append(
                    store.add_evidence(
                        ...
                        source["id"],
                        {
                            ...
                            "confidence": source["final_score"],
                            "metadata": {"url": source["canonical_url"]},
                        },
                    )
                )
        conflicts = (
            self._conflicts(run["id"], sources) if request.include_conflict_detection else []
        )
        summary = f"Collected {len(sources)} deduplicated source(s) and {len(evidence)} citation-ready evidence item(s)."
        final = store.update_run(
            run["id"], status="completed", summary_text=summary, completed_at=store.now()
        )
    ...
    def _official(domain: str) -> bool:
        return (
            domain.endswith((".gov", ".edu", ".org"))
            or domain.startswith("docs.")
            or domain in {"openai.com", "platform.openai.com", "developer.mozilla.org"}
        )
    ...
    def _score(query: str, text: str, domain: str, official: bool, fresh: bool) -> dict:
        ...
        freshness = (
            0.75
            if fresh and re.search(r"\b20\d{2}\b|latest|current|today", text)
            else (0.55 if fresh else 0.5)
        )
        technical = bool(
            re.search(
                r"\b(api|sdk|library|docs?|reference|python|javascript|typescript)\b", query, re.I
            )
        )
        technical_boost = (
            0.12
            if technical
            and (domain.startswith("docs.") or "developer" in domain or "reference" in text)
            else 0.0
        )
        ...
        return {
            ...
            "freshness": freshness,
            "official_source": official_boost,
            "domain_diversity": diversity,
            "technical_doc_boost": technical_boost,
            "penalties": penalties,
            "final": final,
        }
    ...
    def _hydrate_source(self, source: dict, fetch_sources: bool) -> dict:
        ...
        if cached and cached.get("fetched_text"):
            return (
                store.update_source(
                    source["id"],
                    fetched_text=cached["fetched_text"][: self.MAX_FETCH_CHARS],
                    fetched_at=cached.get("fetched_at"),
                    metadata_json=store.json_text(
                        {**source.get("metadata", {}), "cache_reused": True}
                    ),
                )
                or source
            )
        ...
        try:
            ...
            if getattr(page, "fetched", False) and text:
                return (
                    store.update_source(
                        source["id"],
                        fetched_text=safe(text[: self.MAX_FETCH_CHARS]),
            ...
            return (
                store.update_source(
                    ...
                    metadata_json=store.json_text(
                        {
                            **source.get("metadata", {}),
                            "fetch_failure": "No readable content returned.",
                        }
                    ),
                )
                or source
            ...
            return (
                store.update_source(
                    source["id"],
                    metadata_json=store.json_text(
                        {**source.get("metadata", {}), "fetch_failure": safe(str(exc))[:240]}
                    ),
                )
                or source
            )
    ...
    def _readable_text(text: str) -> str:
        text = re.sub(
            r"<script[^>]*>.*?</script>|<style[^>]*>.*?</style>", " ", text or "", flags=re.I | re.S
        )
        text = re.sub(r"<[^>]+>", " ", text)
    ...
    def detail(self, run_id: str) -> dict:
        ...
        return {
            **run,
            "sources": store.related("workspace_web_sources", run_id),
            "evidence": store.related("workspace_web_evidence", run_id),
            "conflicts": store.related("workspace_web_conflicts", run_id),
        }
    ...
    def _conflicts(run_id: str, sources: list[dict]) -> list[dict]:
        ...
        for source in sources:
            match = re.search(
                r"\b\d+(?:\.\d+){1,3}\b", f"{source.get('title', '')} {source.get('snippet', '')}"
            )
            if match:
            ...
            found.append(
                store.add_conflict(
                    run_id,
                    {
                        ...
                        "claim_a": first[0],
                        "claim_b": second[0],
                        "source_ids": [item["id"] for item in first[1] + second[1]],
                        "severity": "medium",
                        "metadata": {"reason": "sources mention different version numbers"},
                    },
                )
            )
        corpus = [
            (
                source,
                f"{source.get('title', '')} {source.get('snippet', '')} {source.get('fetched_text', '')}",
            )
            for source in sources
        ]
                ...
                found.append(
                    store.add_conflict(
                        run_id,
                        {
                            ...
                            "source_ids": [item["id"] for item in first[1] + second[1]],
                            "severity": "medium",
                            "metadata": {"reason": f"sources mention different {topic} values"},
                        },
                    )
                )
        return found
    ...
    @staticmethod
    def _memory(
        run_id: str, sources: list[dict], evidence: list[dict], conflicts: list[dict]
    ) -> None:
        try:
            ...
            for item in evidence[:8]:
                indexer.index_record(
                    ...
                    source_id=item["id"],
                    title=f"Web finding {item['citation_label']}",
                    content=item["evidence_text"],
                    memory_type="research_finding",
                    tags=["web_search", "research_finding"],
                )
                indexer.index_record(
                    scope_type="research_run",
                    scope_id=run_id,
                    source_type="web_search",
                    source_id=f"technical:{item['id']}",
                    title=f"Technical fact {item['citation_label']}",
                    content=item["evidence_text"],
                    memory_type="technical_fact",
                    tags=["web_search", "technical_fact"],
                )
            for source in sources[:8]:
                indexer.index_record(
                    ...
                    scope_id=run_id,
                    source_type="web_search",
                    source_id=f"quality:{source['id']}",
                    title=f"Source quality: {source['title']}",
                    content={
                        "score": source.get("final_score"),
                        "url": source.get("canonical_url"),
                    },
                    memory_type="source_quality_note",
                    tags=["web_search", "source_quality_note"],
                )
            for conflict in conflicts:
                indexer.index_record(
                    ...
                    scope_id=run_id,
                    source_type="web_search",
                    source_id=f"conflict:{conflict['id']}",
                    title=f"Open conflict: {conflict['topic']}",
                    content=conflict,
                    memory_type="conflict",
                    tags=["web_search", "conflict"],
                )
        except Exception:
            pass
