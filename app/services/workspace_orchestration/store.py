...
def conn():
 url=get_settings().database_url; c=sqlite3.connect(url.replace("sqlite:///", "", 1) if url.startswith("sqlite:///") else "neo_memory.db",timeout=30);c.row_factory=sqlite3.Row;return c
...
def insert(table,payload):
 c=conn()
 try:c.execute(f"INSERT INTO {table} ({','.join(payload)}) VALUES ({','.join('?' for _ in payload)})",list(payload.values()));c.commit()
 finally:c.close()
def many(table,wid):
 c=conn()
 order="updated_at" if table=="workspace_orchestration_readiness_checks" else "created_at"
 try:return [row(x) for x in c.execute(f"SELECT * FROM {table} WHERE workspace_id=? ORDER BY {order}",(wid,))]
 finally:c.close()
