...
from app.services.memory_v2.correction_resolver import (
...
from app.services.memory_v2.extraction_diagnostics import (
    ExtractionDiagnosticSink,
    InMemoryExtractionDiagnostics,
)
from app.services.memory_v2.feature_flags import MemoryV2WriteMode
from app.services.memory_v2.grounding import ground_assertion, ground_retraction
...
from app.services.memory_v2.preparser import deterministic_model_response, preparse
...
class MemoryV2ExtractionCoordinator:
    def __init__(
        self,
    ...
    def _gate(
        ...
        if request.owner_id != context.execution.validated_owner():
            ...
            reason = "memory_v2_extraction_disabled"
        elif flags.mode_for(request.owner_id) is not MemoryV2WriteMode.CANONICAL:
            reason = "extraction_requires_canonical_owner"
        elif (
            request.mode is ExtractionMode.FOREGROUND_DETERMINISTIC
            and not flags.foreground_commands_enabled
        ):
    ...
    def _persist_candidate(
        ...
        *,
        ...
        command = PersistExtractionCandidateCommand(
            ...
            extractor_name=(
                "phase4-model" if model_summary.called else "phase4-deterministic-preparser"
            ),
    ...
    def _apply_candidate(
        ...
        *,
        ...
        return ExtractionCandidateDecision(
            ...
            review_required=(
                result.compatibility.review_required if result.compatibility else False
            ),
            memory_ids=(
                tuple(UUID(item) for item in result.compatibility.affected_memory_ids)
