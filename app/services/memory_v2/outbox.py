"""Post-commit leasing and independent derived-target processing for memory v2."""
...
class MemoryV2OutboxProcessor:
    ...
    def lease_batch(
        ...
        *,
        ...
        with self._sessions.begin() as session:
            ...
            events = list(
                session.scalars(
                    select(MemoryOutboxV2)
                    .where(
                        MemoryOutboxV2.owner_id == self.owner_id,
                        MemoryOutboxV2.event_kind.in_(
                            ("canonical_upsert", "canonical_remove", "reconciliation_request")
                        ),
                        MemoryOutboxV2.state.in_(("pending", "failed", "processing")),
                        or_(
                            MemoryOutboxV2.next_retry_at.is_(None),
                            MemoryOutboxV2.next_retry_at <= now,
                        ),
                    )
                    .order_by(MemoryOutboxV2.created_at, MemoryOutboxV2.id)
                    .limit(batch * 3)
                )
            )
            for event in events:
                ...
                for target in targets:
                    result = session.execute(
                        ...
                        .where(
                            MemoryOutboxDeliveryV2.id == target.id,
                            or_(
                                and_(
                                    MemoryOutboxDeliveryV2.state.in_(
                                        (
                                    ...
                                    or_(
                                        MemoryOutboxDeliveryV2.next_attempt_at.is_(None),
                                        MemoryOutboxDeliveryV2.next_attempt_at <= now,
                                    ),
                                ),
                                and_(
                                    MemoryOutboxDeliveryV2.state
                                    == DerivedTargetState.PROCESSING.value,
                                    MemoryOutboxDeliveryV2.lease_expires_at <= now,
                                ),
                            ),
                        )
                        .values(
                            state=DerivedTargetState.PROCESSING.value,
                            attempts=MemoryOutboxDeliveryV2.attempts + 1,
                            worker_id=worker,
                            leased_at=now,
        ...
        with self._sessions.begin() as session:
            result = session.execute(
                ...
                .where(
                    ...
                    MemoryOutboxDeliveryV2.event_id == str(event_id),
                    MemoryOutboxDeliveryV2.target == target.value,
                    MemoryOutboxDeliveryV2.state == DerivedTargetState.DEAD_LETTER.value,
                )
                .values(
            ...
            if result.rowcount:
                session.execute(
                    ...
                    .where(
                        MemoryOutboxV2.owner_id == self.owner_id,
                        MemoryOutboxV2.id == str(event_id),
                    )
        ...
        existing = {
            ...
            for row in session.scalars(
                select(MemoryOutboxDeliveryV2).where(
                    MemoryOutboxDeliveryV2.owner_id == self.owner_id,
                    MemoryOutboxDeliveryV2.event_id == event.id,
                )
            )
        }
        ...
        with self._sessions() as session:
            ...
            event = session.scalar(
                select(MemoryOutboxV2).where(
                    MemoryOutboxV2.owner_id == self.owner_id,
                    MemoryOutboxV2.id == str(lease.event_id),
                )
            )
            if event is None:
        ...
        with self._sessions.begin() as session:
            row = session.scalar(
                select(MemoryDerivedStateV2).where(
                    ...
                    MemoryDerivedStateV2.memory_id == memory_id,
                    MemoryDerivedStateV2.target == target.value,
                )
            )
            values = {
                "state": state.value,
    ...
    def _finish_target(self, lease, target, *, state, error_code):
        ...
        with self._sessions.begin() as session:
            delivery = session.scalar(
                select(MemoryOutboxDeliveryV2).where(
                    MemoryOutboxDeliveryV2.owner_id == self.owner_id,
                    MemoryOutboxDeliveryV2.event_id == str(lease.event_id),
                    MemoryOutboxDeliveryV2.target == target.value,
                    MemoryOutboxDeliveryV2.state == DerivedTargetState.PROCESSING.value,
                    MemoryOutboxDeliveryV2.worker_id == lease.worker_id,
                )
            )
            if delivery is None:
                raise RuntimeError("lease_lost")
    ...
    def _refresh_event_state(self, session, event_id, now):
        ...
        deliveries = list(
            session.scalars(
                select(MemoryOutboxDeliveryV2).where(
                    MemoryOutboxDeliveryV2.owner_id == self.owner_id,
                    MemoryOutboxDeliveryV2.event_id == event_id,
                )
            )
        )
    ...
    def _queue_repair(self, *, memory_id, action, reason, revision, content_hash, target):
        ...
        key = (
            f"phase6:repair:{self.owner_id}:{memory_id}:{action}:"
        ...
        with self._sessions.begin() as session:
            existing = session.scalar(
                select(MemoryOutboxV2).where(
                    MemoryOutboxV2.owner_id == self.owner_id,
                    MemoryOutboxV2.event_idempotency_key == key,
                )
            )
            if existing is not None:
                if existing.state == "done":
                    ...
                    for delivery in session.scalars(
                        select(MemoryOutboxDeliveryV2).where(
                            MemoryOutboxDeliveryV2.owner_id == self.owner_id,
                            MemoryOutboxDeliveryV2.event_id == existing.id,
                        )
                    ):
