...
class OutboxBatch(Phase6Model):
    leases: tuple[OutboxLease, ...]


class OutboxTargetDiagnostic(Phase6Model):
    event_id: UUID
    owner_id: UUID
    memory_id: UUID | None
...
class OutboxProcessResult(Phase6Model):
    event_id: UUID
...
class DerivedDocument(Phase6Model):
...
class ValidatedSemanticCandidate(Phase6Model):
    memory_id: UUID
    normalized_similarity: float = Field(ge=0, le=1)


class IndexRepairRequest(Phase6Model):
    owner_id: UUID
    memory_id: UUID
...
class ReconciliationReport(Phase6Model):
...
class CoverageReport(Phase6Model):
    owner_id: UUID
...
class RebuildResult(Phase6Model):
    owner_id: UUID
...
class RebuildVerification(Phase6Model):
    owner_id: UUID
...
class GlobalCoverageReport(Phase6Model):
...
class ProviderHealth(Phase6Model):
    provider: str
    model: str
...
class SemanticRecallDiagnostic(Phase6Model):
