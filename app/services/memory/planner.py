"""Pure deterministic planner for memory canonical state transitions."""
...
from app.services.memory.tombstones import TombstoneSnapshot, tombstone_expiration
...
def _plan_uuid(operation_id: str, label: str) -> str:
    return str(uuid5(NAMESPACE_URL, f"neo.memory.memory:{operation_id}:{label}"))
...
def _outbox(
    ...
    *,
    ...
    return OutboxIntent(
        ...
        idempotency_key=(
            f"memory:{context.owner_id}:{event_kind}:{owner_key}:"
