...
class MemoryIndexMaintenance:
    ...
    def from_settings(cls, engine: Engine, *, settings: Settings, **dependencies):
        ...
        return cls(
            engine,
            max_attempts=settings.memory_retry_max_attempts,
            reconciliation_batch_size=settings.memory_reconciliation_batch_size,
            alert_oldest_pending_seconds=settings.memory_alert_oldest_pending_seconds,
            alert_dead_letter_count=settings.memory_alert_dead_letter_count,
            alert_min_coverage_ratio=settings.memory_alert_min_coverage_ratio,
            alert_consecutive_provider_failures=(
                settings.memory_alert_consecutive_provider_failures
            ),
            alert_stale_ghost_rate=settings.memory_alert_stale_ghost_rate,
            alert_lease_expiration_rate=settings.memory_alert_lease_expiration_rate,
            **dependencies,
        )
                ...
                derived = list(
                    session.scalars(
                        select(MemoryDerivedState).where(
                            ...
                            MemoryDerivedState.memory_id.in_(checked_ids),
                        )
                    )
                )
                delivery_events = list(
                    session.scalars(
                        select(MemoryOutbox).where(
                            ...
                            MemoryOutbox.memory_id.in_(checked_ids),
                        )
                    )
                )
                    ...
                    deliveries = list(
                        session.scalars(
                            select(MemoryOutboxDelivery).where(
                                ...
                                MemoryOutboxDelivery.event_id.in_(event_ids),
                            )
                        )
                    )
        done_upserts = [
    ...
    def rebuild_owner(self, *, now: datetime | None = None) -> RebuildResult:
        ...
        with self._sessions.begin() as session:
            session.execute(
                update(MemoryDerivedState)
                .where(MemoryDerivedState.owner_id == self.owner_id)
                .values(
            ...
            if documents:
                session.execute(
                    ...
                    .where(
                        MemoryDerivedState.owner_id == self.owner_id,
                        MemoryDerivedState.memory_id.in_(tuple(documents)),
                    )
                    .values(
    ...
    def coverage(self, *, now: datetime | None = None) -> CoverageReport:
        ...
        with self._sessions() as session:
            ...
            states = list(
                session.scalars(
                    select(MemoryDerivedState).where(MemoryDerivedState.owner_id == self.owner_id)
                )
            )
            outbox = list(
                session.scalars(select(MemoryOutbox).where(MemoryOutbox.owner_id == self.owner_id))
            )
            deliveries = list(
                session.scalars(
                    select(MemoryOutboxDelivery).where(
...
class PrivilegedGlobalMemoryMaintenance:
    """Global rebuild requires an explicit administrative capability at construction."""

    def __init__(self, maintainers: list[MemoryIndexMaintenance], *, authorized: bool) -> None:
        if not authorized:
            raise PermissionError("privileged_memory_maintenance_required")
