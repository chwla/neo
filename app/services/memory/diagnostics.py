"""Read-only identity, integrity, and invariant primitives for personal memory."""
...
def _grouped_record_counts(connection, owner_id: str) -> dict[str, int]:
    rows = connection.execute(
        ...
        .group_by(MemoryRecord.status, MemoryRecord.memory_type)
    ).all()
...
def inspect_memory_invariants(engine: Engine, *, owner_id: str) -> MemoryInvariantReport:
    owner = canonical_uuid(owner_id)
    identity = identify_database_owner(engine)
    ...
    if identity.owner_id != owner:
        violations.append(
            InvariantViolation(
                ...
                table=MemoryOwnerBinding.__tablename__,
    ...
    with engine.connect() as connection:
        ...
        for row in duplicate_slots:
            violations.append(
                InvariantViolation(
                    ...
                    table=MemoryRecord.__tablename__,
        ...
        invalid_payloads = connection.execute(
            text(
                "SELECT id FROM memory_records WHERE owner_id = :owner AND NOT ("
        ...
        invalid_candidates = connection.execute(
            text(
                "SELECT id FROM memory_candidates WHERE owner_id = :owner AND NOT ("
                "(sensitivity = 'normal' AND canonical_payload IS NOT NULL "
                "AND display_text IS NOT NULL AND length(trim(display_text)) > 0 "
                "AND encrypted_canonical_payload IS NULL AND encrypted_display_payload IS NULL "
                "AND encryption_algorithm IS NULL AND encryption_key_version IS NULL "
        ...
        if invalid_candidates:
            violations.append(
                InvariantViolation(
                    ...
                    table=MemoryCandidate.__tablename__,
        ...
        missing_operations = connection.execute(
            text(
                ...
                "LEFT JOIN memory_operations o ON o.id = r.created_by_operation_id "
        ...
        orphan_sources = connection.execute(
            text(
                ...
                "LEFT JOIN memory_records r ON r.id = s.memory_id AND r.owner_id = s.owner_id "
                "LEFT JOIN memory_operations o ON o.id = s.operation_id "
        ...
        if orphan_sources:
            violations.append(
                InvariantViolation(
                    ...
                    table=MemorySource.__tablename__,
        ...
        invalid_relations = connection.execute(
            text(
                "SELECT rel.id FROM memory_relations rel "
                "LEFT JOIN memory_records f ON f.id = rel.from_memory_id "
                "AND f.owner_id = rel.owner_id "
                "LEFT JOIN memory_records t ON t.id = rel.to_memory_id "
                "AND t.owner_id = rel.owner_id "
                "LEFT JOIN memory_operations o ON o.id = rel.operation_id "
        ...
        if invalid_relations:
            violations.append(
                InvariantViolation(
                    ...
                    table=MemoryRelation.__tablename__,
        ...
        invalid_candidates_references = connection.execute(
            text(
                ...
                "LEFT JOIN memory_operations o ON o.id = c.applied_operation_id "
        ...
        invalid_outbox_references = connection.execute(
            text(
                "SELECT e.id FROM memory_outbox e "
                "LEFT JOIN memory_records r ON r.id = e.memory_id AND r.owner_id = e.owner_id "
        ...
        if invalid_outbox_references:
            violations.append(
                InvariantViolation(
                    ...
                    table=MemoryOutbox.__tablename__,
        ...
        invalid_tombstone_references = connection.execute(
            text(
                "SELECT t.id FROM memory_tombstones t "
                "LEFT JOIN memory_operations o ON o.id = t.originating_operation_id "
        ...
        if invalid_tombstone_references:
            violations.append(
                InvariantViolation(
                    ...
                    table=MemoryTombstone.__tablename__,
        ...
        pending_outbox = int(
            connection.scalar(
                select(func.count(MemoryOutbox.id)).where(
                    MemoryOutbox.owner_id == owner,
                    MemoryOutbox.state.in_(("pending", "processing")),
                )
            )
            or 0
        )
