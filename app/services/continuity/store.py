...
def rows(table, bid=None):
    c = conn()
    q = f"SELECT * FROM {table}" + (" WHERE bundle_id=?" if bid else "") + " ORDER BY created_at"
    v = [row(x) for x in c.execute(q, (bid,) if bid else ())]
    c.close()
    return v
