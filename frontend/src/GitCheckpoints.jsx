...
export default function GitCheckpoints({ repo, compact = false }) {
  ...
  return <section className={`git-checkpoints ${compact ? "compact" : ""}`}>
    ...
    {selected && <div className="git-checkpoint-detail"><div><strong>{selected.checkpoint.title}</strong><button type="button" onClick={() => setSelected(null)}>Close</button></div><p>Commit: <code>{selected.checkpoint.commit_sha}</code><br />Status: {selected.checkpoint.status}<br />{selected.checkpoint.message || "No message."}</p><ul>{selected.checkpoint.changed_files.map((item) => <li key={item.path}>{item.path}</li>)}</ul><pre>{selected.checkpoint.stats?.summary || "No stats."}</pre><div className="git-operation-history"><strong>Operation history</strong>{selected.operations?.length ? <ul>{selected.operations.map((item) => <li key={item.id}><span>{item.operation_type}</span> · {item.status} · {new Date(item.created_at).toLocaleString()}</li>)}</ul> : <p>No linked operations.</p>}</div><button type="button" className="neo-button danger" disabled={busy} onClick={() => restore(selected.checkpoint)}>Restore managed copy</button></div>}
