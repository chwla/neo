...
export default function Gallery({ onBack, onOpenChat, initialItemId = null }) {
  ...
  /* Facets describe the whole collection, so they are read only from a listing
     that the facet itself is not narrowing. Computed from `items` unconditionally,
     picking a tag would leave the tag bar showing just that tag, and picking an
     origin would collapse the menu to the one option already chosen. */
  ...
  useEffect(() => {
    ...
    setTags([...counts.entries()].sort((a, b) => b[1] - a[1]).slice(0, 12));
  }, [items, tag]);

  const [origins, setOrigins] = useState([]);
  useEffect(() => {
    if (origin) return;
    setOrigins([...new Set(items.map((item) => item.origin).filter(Boolean))].sort());
  }, [items, origin]);
            ...
            <div className="gal-control-group">
              ...
              {origins.length > 1 && (
                <select value={origin} onChange={(event) => setOrigin(event.target.value)} aria-label="Where it came from">
                  ...
                  {origins.map((value) => (
                    <option key={value} value={value}>{ORIGIN_LABEL[value] || value}</option>
                  ))}
                </select>
              )}
              <button
