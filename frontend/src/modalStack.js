 ...
 *
 * It routes Escape and nothing else. It briefly also flagged the document while
 * anything was open, so the background engine could stop painting a field it
 * thought was covered -- which was wrong twice over. A popover is on this stack
 * too: the composer's "+" menu and every row's "..." register here purely so one
 * Escape closes the top thing rather than the whole pile, and neither of them
 * covers anything. And even for a real dialog, stopping the field was solving a
 * rendering cost by taking away the feature that cost was paying for. The field
 * now runs whatever is open; see `backgrounds/engine.js`, which stops for a
 * hidden tab and for nothing else.
 */
