...
export default function PatchApplications({ fileId = null, taskId = null, projectId = null, agentRunId = null, artifactId = null, repoId = null, refreshKey = 0 }) {
  ...
  return <section className="patch-applications">
    ...
    {selected ? <div className="patch-application-detail">
      ...
      <div className="patch-snapshot-downloads"><a href={api.patchApplicationDownloadUrl(selected.id)}>Download audit bundle</a><a href={api.patchApplicationDownloadUrl(selected.id, "original")}>Download primary original</a>{selected.new_content ? <a href={api.patchApplicationDownloadUrl(selected.id, "current")}>Download primary current</a> : null}</div>
      {selected.files?.length ? <div className="patch-file-list"><strong>Changed files ({selected.files.length})</strong>{selected.files.map((file) => <div key={file.id}><code>{file.relative_path}</code> <span>{file.change_type}</span> <span className={`patch-application-status ${file.status}`}>{file.status}</span><small>{shortHash(file.original_sha256)} → {shortHash(file.new_sha256)}</small>{file.error ? <div className="task-error">{file.error}</div> : null}<div className="patch-snapshot-grid"><label>Before<pre>{file.original_content || "New file"}</pre></label><label>After<pre>{file.new_content || "Not applied"}</pre></label></div></div>)}</div> : null}
      ...
      {!selected.files?.length ? <div className="patch-snapshot-grid"><label>Before<pre>{selected.original_content}</pre></label><label>After<pre>{selected.new_content || "Not applied"}</pre></label></div> : null}
    </div> : null}
