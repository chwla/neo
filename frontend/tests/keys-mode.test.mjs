 ...
 *
 * The headline property is at the bottom and it is the reason single-letter
 * bindings are safe to ship switched on: over the real catalogue, no bare default
 * fires while a text field has focus. Everything above it exists to make that
 * hold -- one classifier instead of the three different regexes the app had
 * grown, computed from focus rather than remembered, so there is no stored state
 * anybody can be stranded in.
 */
...
import { allowsBareKeys, focusKind } from "../src/keys/mode.js";
...
describe("there is no mode to be in", () => {
  test("whether a bare key acts is a function of focus and nothing else", () => {
    // The safety argument, stated as a property: the answer takes one argument,
    // so no history of keypresses can produce a different one.
    assert.equal(allowsBareKeys.length, 1);
    assert.equal(allowsBareKeys("text"), false);
    assert.equal(allowsBareKeys("none"), true);
  });
});
...
describe("no bare default can fire while somebody is typing", () => {
  ...
  test("every bare default is inert in a text field", () => {
    // The whole safety argument for shipping bare letters on by default.
  ...
  test("a modifier chord still works from a control", () => {
